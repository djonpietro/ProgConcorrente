  ...
  *prime_counter = 0;
  while (1) {
    ...
    if (elem == -1) {           // recebeu um poison pill. Hora de parar
      ...
      *prime_counter += 1;
      // a seção abaixo serve para guardar os primos gerados
      // sem_wait(&mutex_log);
      // primes[pos] = elem;
      // pos++;
      // sem_post(&mutex_log);
...
int main(int argc, char *argv[]) {
  ...
  // vetor para guardar os primos gerados para fins de depuração
  ...
  // for (int i = 0; i < result.total_prime_count; i++) {
  //   printf("%d ", primes[i]);
  // }
  // printf("\n");
